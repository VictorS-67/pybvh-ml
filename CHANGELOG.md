...
- **Documentation site** at <https://victors-67.github.io/pybvh-ml/> — MkDocs Material + mkdocstrings, the same stack as pybvh's docs: quick start, a user guide (tensor layouts, preprocessing, augmentation, PyTorch integration, skeleton graphs), a dedicated [0.4.0 → 0.5.0 migration page](https://victors-67.github.io/pybvh-ml/migration/) with before/after snippets, and a full API reference generated from the docstrings. Deployed to GitHub Pages on push to `main`, with `mkdocs build --strict` gating broken links; a new `docs` extra installs the toolchain. The README slims to a landing page pointing at the site.
