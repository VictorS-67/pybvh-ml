...
| Change | Migration |
|---|---|
| **Array functions take and return a `MotionArrays`** instead of loose `(root_pos=, joint_data=)` / a 2-tuple | Build it once — `arrays = MotionArrays(root_pos=rp, joint_rot=jd)` — and pass it positionally to every augmentation, the pipeline, and the packers; read `out.root_pos` / `out.joint_rot` back. `rp, jd = pipeline(...)` no longer unpacks and fails loudly. |
...
| Invalid augmentation inputs raise `ValueError` (negative `sigma` on either noise function; `drop_rate` outside `[0, 1)`; mismatched `root_pos` / `joint_data` frame counts; empty joint arrays in `rotate_vertical`; zero-norm quaternions in `add_joint_rotation_noise`) | Fix the offending values — they were silent no-ops or opaque errors before. |
