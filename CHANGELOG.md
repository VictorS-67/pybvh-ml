...
| Change | Migration |
|---|---|
| Requires **pybvh >= 0.8.1, < 0.9** | Upgrade pybvh first; pybvh 0.8.0 ships its own consolidated migration table in its CHANGELOG. |
...
- **Minimum pybvh version: `>=0.8.1,<0.9`.** pybvh 0.8.0 renames the representation surface; pybvh-ml adopts the new names atomically with no dual-name support. The floor is 0.8.1 rather than 0.8.0 because the augmentation axis parsing now calls `pybvh.parse_axis`, which 0.8.1 promoted to the public surface.
