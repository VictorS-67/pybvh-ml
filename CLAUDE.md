...
6. **No global state, no import-time side effects.** pybvh-ml never mutates process-wide numeric or environment state: not `torch.set_float32_matmul_precision`, `use_deterministic_algorithms`, `set_default_dtype`, `set_num_threads`, `manual_seed` or anything under `torch.backends`; not `np.seterr`, `np.random.seed` or print options; not `os.environ`, `warnings.filters` or logging configuration. Precision, determinism, threading and seeding are **application** policy — a data library that sets them makes `import pybvh_ml` change a model's numbers, which is the most insidious class of bug it could ship, since the flag is invisible at every call site and survives every abstraction the application puts between itself and us. The rule holds at call time as well as import time: randomness is passed in (`rng=`, `seed=`), never installed. Enforced by `tests/test_no_global_state_mutation.py` (an AST scan plus a differential import probe), because a convention this easy to break "helpfully" needs a test and not a paragraph.
